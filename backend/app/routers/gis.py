...
LAYERS = [
    ...
    # Live open-source & Indian Government data feeds (GIS Intelligence page only).
    {"id": "seismic", "label": "Seismic Triggers (NCS / USGS)",
     "description": "Real-time earthquake triggers and rupture shaking radius in NER from NCS India & USGS.",
     "available": True, "default_on": True, "group": "RISK",
     "source_hint": "GET /api/gis/layers/seismic -> FeatureCollection<Point>"},
    {"id": "soil_moisture", "label": "Soil Moisture & Saturation (Open-Meteo / IMD)",
     "description": "Multi-depth root-zone soil saturation (0-100cm) and physical Factor of Safety (FS).",
     "available": True, "default_on": True, "group": "RISK",
     "source_hint": "GET /api/gis/layers/soil_moisture -> FeatureCollection<Point>"},
    {"id": "gsi_susceptibility", "label": "GSI Susceptibility (Bhukosh NLSM)",
     "description": "Geological Survey of India 1:50k NLSM susceptibility zones and historical slide inventory.",
     "available": True, "default_on": False, "group": "RISK",
     "source_hint": "GET /api/gis/layers/gsi_susceptibility -> FeatureCollection<Polygon,Point>"},
    {"id": "osm_infrastructure", "label": "Live Lifelines (OpenStreetMap)",
     "description": "Hospitals, emergency relief camps, and bridges in Kohima along NH-29.",
     "available": True, "default_on": False, "group": "ASSETS",
     "source_hint": "GET /api/gis/layers/osm_infrastructure -> FeatureCollection<Point>"},
]
...
def layer(
    ...
    if layer_id == "seismic":
        return respond(gis_store.vector("seismic"), case, response)

    if layer_id == "soil_moisture":
        return respond(gis_store.vector("soil_moisture"), case, response)

    if layer_id == "gsi_susceptibility":
        return respond(gis_store.vector("gsi_susceptibility"), case, response)

    if layer_id == "osm_infrastructure":
        return respond(gis_store.vector("osm_infrastructure"), case, response)

    raise HTTPException(404, f"Layer {layer_id} has no server implementation")


@router.get("/gis/live/summary")
def get_live_summary(response: Response, case: str | None = Depends(get_case)):
    """Summary of live open-source & Indian Govt telemetry (NCS, USGS, Open-Meteo, GSI, OSM)."""
    return respond(gis_store.live_summary(), case, response)


@router.post("/gis/live/refresh")
def refresh_live_feeds(response: Response, case: str | None = Depends(get_case)):
    """Trigger on-demand live re-fetch from NCS India, USGS, Open-Meteo, GSI, and OSM."""
    try:
        import sys
        gis_src = Path(__file__).resolve().parents[3] / "gis" / "src"
        if str(gis_src) not in sys.path:
            sys.path.insert(0, str(gis_src))
        from gis.open_data_engine import OpenDataEngine
        engine = OpenDataEngine()
        summary = engine.run_all()
        return respond(summary, case, response)
    except Exception as e:
        return respond({"status": "error", "message": str(e)}, case, response)
