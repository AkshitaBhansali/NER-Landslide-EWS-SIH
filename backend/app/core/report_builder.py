...
# --------------------------------------------------------------------------- #
# Geo projection helpers — used by the terrain and exposure pages to plot the
# report's *actual* zone polygons and asset coordinates for the current scope,
# rather than a fixed schematic that only relabels per district.
# --------------------------------------------------------------------------- #
...
def _geo_bounds(points: list[tuple[float, float]]) -> tuple[float, float, float, float]:
    """(min_lat, max_lat, min_lng, max_lng) with a small padding, guarding against
    a single-point or degenerate scope (one zone, one asset)."""
    ...
    lat_pad = (max_lat - min_lat) * 0.15
    lng_pad = (max_lng - min_lng) * 0.15
    return (min_lat - lat_pad, max_lat + lat_pad, min_lng - lng_pad, max_lng + lng_pad)


def _projector(points: list[tuple[float, float]], w: float, h: float):
    """Returns a fn(lat, lng) -> (x, y) that fits `points` into a w x h box,
    north-up, aspect-corrected for latitude (equirectangular, fine at this scale)."""
    min_lat, max_lat, min_lng, max_lng = _geo_bounds(points)
    lat_span = max(max_lat - min_lat, 1e-6)
    lng_span = max(max_lng - min_lng, 1e-6)

    def project(lat: float, lng: float) -> tuple[float, float]:
        x = (lng - min_lng) / lng_span * w
        y = (1 - (lat - min_lat) / lat_span) * h
        return round(x, 1), round(y, 1)

    return project, (min_lat, max_lat, min_lng, max_lng)


def _polygon_area_km2(coords: list[list[float]]) -> float:
    """Shoelace formula on a lng/lat ring, converted to km via a local
    equirectangular approximation. Good enough at slope-unit scale (a few km²)."""
    if len(coords) < 3:
        return 0.0
    lat0 = sum(c[1] for c in coords) / len(coords)
    km_lng = _km_per_deg_lng(lat0)
    pts = [(c[0] * km_lng, c[1] * KM_PER_DEG_LAT) for c in coords]
    area = 0.0
...
DISTRICT_DETAILS = {
    "Dima Hasao": {
        "corridor": "NH-27 alignment through the valley",
        "cluster1": "Maibang cluster",
        "cluster2": "Haflong ridge",
        "road": "NH-27",
        "slope": "34",
        "elevation": "0.4\u20131.9",
        "area": "210",
        "hospital": "Hospital \u00b7 Haflong",
        "school": "School cluster",
        "village1": "Village \u00b7 Maibang",
        "bridge": "Bridge \u00b7 NH-27",
        "village2": "Village \u00b7 Diyung",
        "asset1": "NH-27 \u00b7 Maibang\u2013Haflong",
        "asset2": "Haflong Civil Hospital",
        "asset3": "Maibang Primary School",
    },
    "East Khasi Hills": {
        "corridor": "NH-206 Shillong\u2013Dawki alignment across the plateau",
        "cluster1": "Cherrapunji sector",
        "cluster2": "Shillong ridge",
        "road": "NH-206",
        "slope": "38",
        "elevation": "0.6\u20131.8",
        "area": "345",
        ...
        "school": "School cluster",
        "village1": "Village \u00b7 Cherra",
        "bridge": "Bridge \u00b7 Umngot",
        "village2": "Village \u00b7 Mawlynnong",
        "asset1": "NH-206 \u00b7 Shillong\u2013Dawki",
        "asset2": "Shillong Civil Hospital",
        "asset3": "Cherrapunji Higher Secondary",
    },
    "Aizawl": {
        "corridor": "NH-54 alignment through the steep hill slopes",
        "cluster1": "Bawngkawn sector",
        "cluster2": "Zarkawt ridge",
        "road": "NH-54",
        "slope": "41",
        "elevation": "0.5\u20131.6",
        "area": "280",
        "hospital": "Civil Hospital \u00b7 Aizawl",
        "school": "College cluster",
        "village1": "Village \u00b7 Sairang",
        "bridge": "Bridge \u00b7 Tuirial",
        "village2": "Village \u00b7 Durtlang",
        "asset1": "NH-54 \u00b7 Aizawl\u2013Lunglei",
        "asset2": "Aizawl Civil Hospital",
        "asset3": "Zarkawt Govt School",
    },
    "Gangtok": {
        "corridor": "NH-10 Gangtok\u2013Siliguri alignment along the mountain face",
        "cluster1": "Deorali sector",
        "cluster2": "Tashi View ridge",
        "road": "NH-10",
        "slope": "44",
        "elevation": "0.8\u20132.2",
        "area": "190",
        "hospital": "STNM Hospital \u00b7 Gangtok",
        "school": "School sector",
        "village1": "Village \u00b7 Rumtek",
        "bridge": "Bridge \u00b7 Teesta",
        "village2": "Village \u00b7 Ranipool",
        "asset1": "NH-10 \u00b7 Gangtok\u2013Siliguri",
        "asset2": "STNM Central Hospital",
        "asset3": "Deorali Secondary School",
    },
    "Kohima": {
        "corridor": "NH-29 Kohima\u2013Dimapur bypass corridor",
        "cluster1": "Phesama sector",
        "cluster2": "Jotsoma ridge",
        "road": "NH-29",
        "slope": "36",
        "elevation": "0.7\u20131.7",
        "area": "310",
        "hospital": "Naga Hospital \u00b7 Kohima",
        "school": "Model School",
        "village1": "Village \u00b7 Khonoma",
        "bridge": "Bridge \u00b7 Zubza",
        "village2": "Village \u00b7 Chiephobozou",
        "asset1": "NH-29 \u00b7 Kohima\u2013Dimapur",
        "asset2": "Naga Hospital Authority",
        "asset3": "Kohima Central School",
    },
}

def get_district_info(scope_name: str) -> dict:
    for key, val in DISTRICT_DETAILS.items():
        if key.lower() in scope_name.lower():
            return val
    
    seed = sum(ord(c) for c in scope_name)
    slope = 28 + (seed % 18)
    elev_min = round(0.3 + (seed % 5) * 0.1, 1)
    elev_max = round(elev_min + 1.2 + (seed % 8) * 0.1, 1)
    area = 150 + (seed % 250)
    highway_num = 10 + (seed % 80)
    
    clean = scope_name.replace(" District", "").replace(" Metropolitan", "").replace(" Region \u2014 all states", "").strip()
    if not clean or "North Eastern" in clean:
        clean = "Dima Hasao"

    return {
        "corridor": f"NH-{highway_num} alignment through the {clean} sector",
        "cluster1": f"{clean} North cluster",
        "cluster2": f"{clean} ridge",
        "road": f"NH-{highway_num}",
        "slope": str(slope),
        "elevation": f"{elev_min}\u2013{elev_max}",
        "area": str(area),
        "hospital": f"District Hospital \u00b7 {clean}",
        "school": f"{clean} School Sector",
        "village1": f"Village \u00b7 {clean} West",
        "bridge": f"Bridge \u00b7 NH-{highway_num}",
        "village2": f"Village \u00b7 {clean} East",
        "asset1": f"NH-{highway_num} \u00b7 {clean} Highway",
        "asset2": f"{clean} Civil Hospital",
        "asset3": f"{clean} Primary School",
    }


def _spatial(report, corridor, title) -> str:
    scope_name = report.get("scope", "Dima Hasao")
    zones_geo = report.get("zones_geo") or []

    if not zones_geo:
        # No risk zones registered in this scope — nothing real to project.
        return f"""
<section class="page">
  <div class="sec-head"><div><p class="eyebrow">04 \u2014 The terrain</p><h2 class="sec-title">Topographical risk \u2014 {e(scope_name)}</h2></div><div class="mono muted" style="font-size:11px;">hillshade + risk</div></div>
  <p class="sub" style="margin-top:0;margin-bottom:16px;">No risk zones are registered for {e(scope_name)} in this period.</p>
  <div class="card soft" style="padding:24px;text-align:center;color:#6b7b77;">No slope units to plot for this scope.</div>
  {foot(title, 5, ['GIS'])}
</section>"""

    w, h = 680, 340
    points = [(z["lat"], z["lng"]) for z in zones_geo]
    project, (min_lat, max_lat, min_lng, max_lng) = _projector(points, w, h)

    # Real per-zone stats for this scope, not a name-seeded placeholder.
    slopes = [z["slope_deg"] for z in zones_geo if z.get("slope_deg")]
    elevations = [z["elevation_m"] for z in zones_geo if z.get("elevation_m")]
    mean_slope = round(sum(slopes) / len(slopes)) if slopes else 0
    elev_min = round(min(elevations) / 1000, 1) if elevations else 0
    elev_max = round(max(elevations) / 1000, 1) if elevations else 0
    high_risk_zones = [z for z in zones_geo if z.get("risk_level") in ("HIGH", "CRITICAL")]
    high_risk_area = sum(
        _polygon_area_km2(z["geometry"]["coordinates"][0])
        for z in high_risk_zones
        if (z.get("geometry") or {}).get("type") == "Polygon"
    )

    # Actual road alignments (from the GIS vector store) that fall inside this
    # scope's bounding box, so the "corridor of concern" line is the real road.
    road_paths = []
    try:
        from . import gis_store
        roads_fc = gis_store.vector("roads")
        for feat in roads_fc.get("features", []):
            ...
            if min_lat - 0.1 <= mid_lat <= max_lat + 0.1 and min_lng - 0.1 <= mid_lng <= max_lng + 0.1:
                svg_pts = [project(c[1], c[0]) for c in coords]
                road_paths.append((svg_pts, (feat.get("properties") or {}).get("name", "Road")))
    except Exception:
        pass

    zone_blobs = []
    zone_labels = []
    for z in sorted(zones_geo, key=lambda z: z["risk_score"]):
        cx, cy = project(z["lat"], z["lng"])
        band = z.get("risk_level") or _band(z["risk_score"])
        colour = LEVEL_COLOR.get(band, "#6b7b77")
        # Radius reflects how far above background the zone's score is —
        # sized by data, not a fixed schematic blob.
        r = 34 + z["risk_score"] * 0.9
        zone_blobs.append(
            f'<circle cx="{cx}" cy="{cy}" r="{r:.0f}" fill="{colour}" opacity="0.32"/>'
        )
        zone_labels.append(
            f'<circle cx="{cx}" cy="{cy}" r="4" fill="{colour}"/>'
            f'<text x="{cx + 9}" y="{cy - 4}">{e(z["name"])}</text>'
            f'<text x="{cx + 9}" y="{cy + 9}" fill="#5a6b64">{z["risk_score"]:.0f} \u00b7 {e(band.title())}</text>'
        )

    road_svg = []
    for i, (svg_pts, name) in enumerate(road_paths[:3]):
        d = " ".join(f'{"M" if i == 0 else "L"}{x} {y}' for i, (x, y) in enumerate(svg_pts))
        road_svg.append(f'<path d="{d}" fill="none" stroke="#1f2a28" stroke-width="3.5" opacity=".8"/>')
        road_svg.append(f'<path d="{d}" fill="none" stroke="#f3f0e6" stroke-width="1.2" stroke-dasharray="7 6"/>')
        mx, my = svg_pts[len(svg_pts) // 2]
        road_svg.append(
            f'<rect x="{mx - 4}" y="{my - 4}" width="9" height="9" fill="#123f39"/>'
            f'<text x="{mx + 10}" y="{my + 4}">{e(name)}</text>'
        )

    corridor_note = (
        road_paths[0][1] if road_paths else "no registered highway alignment in scope"
    )

    return f"""
<section class="page">
  <div class="sec-head"><div><p class="eyebrow">04 \u2014 The terrain</p><h2 class="sec-title">Topographical risk \u2014 {e(scope_name)}</h2></div><div class="mono muted" style="font-size:11px;">hillshade + risk</div></div>
  <p class="sub" style="margin-top:0;margin-bottom:16px;">Risk draped over the slopes it sits on. Steeper, darker faces carry the highest risk; the corridor of concern follows {e(corridor_note)}.</p>

  <div class="card soft" style="padding:12px;">
    <svg width="100%" viewBox="0 0 {w} {h}" style="border-radius:8px; display:block;">
      <defs>
        <linearGradient id="slopeG" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0" stop-color="#cfe0d6"/><stop offset="1" stop-color="#9fb8ab"/>
        </linearGradient>
      </defs>
      <rect width="{w}" height="{h}" fill="url(#slopeG)"/>
      <g fill="none" stroke="#6f8a7c" stroke-width="1" opacity="0.4">
        <path d="M0 60 C120 30 240 90 360 60 S560 20 {w} 55"/>
        <path d="M0 145 C130 116 250 172 370 145 S560 108 {w} 140"/>
        <path d="M0 245 C130 216 260 270 380 245 S560 208 {w} 240"/>
      </g>
      {"".join(zone_blobs)}
      {"".join(road_svg)}
      <g font-family="IBM Plex Mono, monospace" font-size="10" fill="#1f2a28">
        {"".join(zone_labels)}
      </g>
    </svg>
    <p class="cap">Real slope-unit positions and scores for {e(scope_name)}, projected from recorded coordinates{" with the registered highway alignment overlaid" if road_paths else ""}. {len(zones_geo)} zone(s) in scope.</p>
  </div>

  <div class="grid-3" style="margin-top:18px;">
    <div class="kpi"><div class="lab">Mean slope, all zones</div><div class="val">{mean_slope}<small>\u00b0</small></div><div class="delta muted">DEM-derived</div></div>
    <div class="kpi"><div class="lab">Elevation range</div><div class="val">{elev_min}\u2013{elev_max}<small>km</small></div><div class="delta muted">valley to ridge</div></div>
    <div class="kpi"><div class="lab">High-risk area</div><div class="val">{high_risk_area:.0f}<small>km\u00b2</small></div><div class="delta muted">{len(high_risk_zones)} zone(s) \u2265 HIGH</div></div>
  </div>
...
def _exposure(report, title) -> str:
    scope_name = report.get("scope", "Dima Hasao")
    assets = report.get("exposure_detail") or []
    zones_geo = report.get("zones_geo") or []

    if not assets:
        return f"""
<section class="page">
  <div class="sec-head"><div><p class="eyebrow">05 \u2014 Who is exposed</p><h2 class="sec-title">Infrastructure exposure</h2></div><div class="mono muted" style="font-size:11px;">risk \u00d7 importance</div></div>
  <p class="sub" style="margin-top:0;margin-bottom:16px;">No infrastructure assets are registered for {e(scope_name)}.</p>
  <div class="card soft" style="padding:24px;text-align:center;color:#6b7b77;">No assets to plot for this scope.</div>
  {foot(title, 6, ['GIS'])}
</section>"""

    w, h = 680, 320
    # Project asset points and any registered zones together so the exposure
    # map frames the same real ground the terrain page just described.
    points = [(a["location"]["lat"], a["location"]["lng"]) for a in assets]
    points += [(z["lat"], z["lng"]) for z in zones_geo]
    project, _ = _projector(points, w, h)

    ICONS = {
        "HOSPITAL": ("triangle", "#7d211c"),
        "SCHOOL": ("square", "#8f3f18"),
        "VILLAGE": ("circle", "#123f39"),
        "BRIDGE": ("diamond", "#5a2b8f"),
        "HIGHWAY": ("diamond", "#5a2b8f"),
    }
    RISK_ZONE_FILL = {"LOW": "#4a9d4a", "MODERATE": "#e6a52a", "HIGH": "#d1642e", "CRITICAL": "#b5342e"}

    zone_fields = []
    for z in zones_geo:
        cx, cy = project(z["lat"], z["lng"])
        colour = RISK_ZONE_FILL.get(z.get("risk_level"), "#6b7b77")
        r = 46 + z["risk_score"] * 0.7
        zone_fields.append(f'<circle cx="{cx}" cy="{cy}" r="{r:.0f}" fill="{colour}" opacity="0.16"/>')

    markers = []
    for a in assets[:8]:
        cx, cy = project(a["location"]["lat"], a["location"]["lng"])
        shape, colour = ICONS.get(a["type"], ("circle", "#123f39"))
        if shape == "triangle":
            glyph = f'<path transform="translate({cx},{cy})" d="M0 -7 L6 5 L-6 5 Z" fill="{colour}"/>'
        elif shape == "square":
            glyph = f'<rect x="{cx - 5}" y="{cy - 5}" width="10" height="10" fill="{colour}"/>'
        elif shape == "diamond":
            glyph = f'<path transform="translate({cx},{cy})" d="M-6 0 L0 -6 L6 0 L0 6 Z" fill="{colour}"/>'
        else:
            glyph = f'<circle cx="{cx}" cy="{cy}" r="5" fill="{colour}"/>'
        markers.append(f'{glyph}<text x="{cx + 9}" y="{cy + 4}">{e(a["name"])}</text>')

    high_risk_label = ""
    if zones_geo:
        top_zone = max(zones_geo, key=lambda z: z["risk_score"])
        tx, ty = project(top_zone["lat"], top_zone["lng"])
        high_risk_label = f'<text x="{tx - 60}" y="{ty - 40}" fill="#7d211c">HIGH-RISK ZONE</text>'

    def pill(level: str) -> str:
        cls = {"CRITICAL": "p-sev", "HIGH": "p-high", "MODERATE": "p-mod"}.get(level, "p-mod")
        return f'<span class="pill {cls}">{e(level.title())}</span>'

    ACTION = {
        "HIGHWAY": "Pre-position clearance team",
        "BRIDGE": "Inspect structural integrity",
        "HOSPITAL": "Verify emergency access road",
        "SCHOOL": "Issue precautionary advisory",
        "VILLAGE": "Confirm evacuation route",
    }

    rows = []
    for i, a in enumerate(assets[:6], start=1):
        rows.append(
            f'<tr><td>{i}</td><td>{e(a["name"])}</td><td>{e(a["type"].title())}</td>'
            f'<td>{pill(a["risk_level"])}</td><td>{e(ACTION.get(a["type"], "Monitor and reassess"))}</td></tr>'
        )

    return f"""
<section class="page">
  <div class="sec-head"><div><p class="eyebrow">05 \u2014 Who is exposed</p><h2 class="sec-title">Infrastructure exposure</h2></div><div class="mono muted" style="font-size:11px;">risk \u00d7 importance</div></div>
  <p class="sub" style="margin-top:0;margin-bottom:16px;">The risk field with strategic assets plotted on top for {e(scope_name)}. A high-risk zone matters most where it overlaps roads, hospitals, schools and dense settlement \u2014 those overlaps drive the priority list below.</p>

  <div class="card soft" style="padding:12px;">
    <svg width="100%" viewBox="0 0 {w} {h}" style="display:block; border-radius:8px;">
      <rect width="{w}" height="{h}" fill="#eef2ec"/>
      {"".join(zone_fields)}
      <g font-family="IBM Plex Mono, monospace" font-size="9.5" fill="#16211f">
        {"".join(markers)}
      </g>
      <text font-family="IBM Plex Mono,monospace" font-size="10">{high_risk_label}</text>
    </svg>
    <p class="cap">Real asset coordinates for {e(scope_name)}, plotted over the current risk-zone footprint. {len(assets)} asset(s) ranked by exposure score.</p>
  </div>

  <table class="dt" style="margin-top:18px;">
    <tr><th>Priority</th><th>Asset</th><th>Type</th><th>Zone Risk</th><th>Recommended Action</th></tr>
    {"".join(rows)}
  </table>
