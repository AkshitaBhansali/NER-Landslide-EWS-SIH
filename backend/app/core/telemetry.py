...
def tick(db: Session) -> dict:
    ...
    for did, st in states.items():
        ...
        db.add(WeatherObservation(
            ...
            forecast=[
                {
                    "date": (now + timedelta(days=idx + 1)).isoformat(),
                    "rainfall_mm": round(r24 * mult, 1),
                    "probability_pct": int(min(96, max(8, (r24 * mult / 70.0) * 90))),
                    "risk_level": ("CRITICAL" if (r24 * mult / 70.0) >= 1.15 else
                                   "HIGH" if (r24 * mult / 70.0) >= 0.85 else
                                   "MODERATE" if (r24 * mult / 70.0) >= 0.5 else "LOW"),
                    "temp_min": 19,
                    "temp_max": 28,
                }
                for idx, mult in enumerate([1.34, 0.39, 0.83, 1.21, 0.53])
            ],
